...
class GenerateurFugue:
    ...
    def to_ly_abs(self, notes):
        ...
        for n in notes:
            ...
            # Forcer la note dans la gamme pour la traduction LilyPond
            pitch_propre = self.corriger_pitch_diatonique(n.pitch)
            
            # Trouver l'index dans la gamme chromatique générale pour nommer la note
            nom = self.chroma[pitch_propre % 12]
            
            # Ajustement selon notre dictionnaire local de tonalité
            if pitch_propre % 12 in self.gamme_midi:
                idx = self.gamme_midi.index(pitch_propre % 12)
                nom = self.gamme_ly[idx]
                
            oct_val = (pitch_propre // 12) - 5
    ...
    def transposition(self, notes, intervalle=None):
        ...
        return [Note(self.corriger_pitch_diatonique(n.pitch + intervalle) if n.pitch != -1 else -1, n.duration) for n in notes]
    ...
    def inversion(self, notes):
        ...
        return [Note(self.corriger_pitch_diatonique(pivot - (n.pitch - pivot)) if n.pitch != -1 else -1, n.duration) for n in notes]
    ...
    def composer(self, theme_ly, bpm, poids_fonctions, duree_cible_secondes):
        ...
        for transf in pool_fonctions_base:
            ...
            if duree_accumulee + duree_seq > duree_cible_secondes: break
        ...
        print(f"-> Fugue en {self.tonalite.upper()} {self.mode}. Durée : {round(duree_accumulee, 2)}s")
        return " | \n  ".join(partition)
...
theme_input = "r8 d'8 e'8 f'8 g'8 f'8 g'8 a'8 bes'8" 

configuration_fonctions = {
    "transposition": 3, "augmentation": 2, "diminution": 1,
    "inversion": 3, "retrograde": 2, "hauteurs_retrogrades": 1, "durees_retrogrades": 1
}
