...
class GenerateurFugue:
    ...
    def composer(self, theme_ly, bpm, poids_fonctions, poids_combinaisons, duree_cible_secondes):
        sujet = self.parse_ly(theme_ly)
        duree_sujet_initiale = self.calculer_duree_sequence(sujet, bpm)
        ...
        # 1. Analyse et enregistrement des fonctions simples avec leur nom
        ...
        # 2. Analyse et enregistrement des combinaisons avec leurs noms
        ...
        for block in pool_blocks:
            # --- LE GARDE-FOU INTELLIGENT DE DURÉE ---
            # On simule la durée du bloc AVANT de l'appliquer réellement
            sequence_test = block["func"](sujet)
            duree_bloc = self.calculer_duree_sequence(sequence_test, bpm)
            
            # Si ajouter ce bloc nous fait dépasser la durée cible de trop (marge de 2s)
            if duree_accumulee + duree_bloc > duree_cible_secondes + 2:
                # L'IA sait que ce bloc (ex: une double augmentation) est trop long !
                # Elle cherche alors dans le reste du pool s'il n'y a pas un bloc plus court (ex: une diminution)
                bloc_de_secours = None
                for b_secours in pool_blocks:
                    if b_secours not in partition: # Pas encore utilisé
                        seq_secours = b_secours["func"](sujet)
                        duree_secours = self.calculer_duree_sequence(seq_secours, bpm)
                        if duree_accumulee + duree_secours <= duree_cible_secondes + 2:
                            bloc_de_secours = b_secours
                            break
                
                if bloc_de_secours:
                    # On remplace le gros bloc par le bloc plus court trouvé
                    block = bloc_de_secours
                    sequence_test = block["func"](sujet)
                    duree_bloc = self.calculer_duree_sequence(sequence_test, bpm)
                else:
                    # Aucun bloc ne rentre sans déborder, on stoppe la fugue proprement ici
                    break
            
            # Application réelle du bloc choisi (ou du bloc de secours)
...
configuration_combinaisons = {
    ("inversion", "retrograde"): 0,       # L'inversion-rétrograde célèbre de Bach
    ("transposition", "inversion"): 2,   # Inverse le thème et le transpose (idéal pour les réponses de fugue)
    ("transposition", "augmentation"): 2,   # Inverse le thème et le transpose (idéal pour les réponses de fugue)
